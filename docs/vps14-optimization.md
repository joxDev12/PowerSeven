...
Fonte: discovery read-only Azure del 26 settembre 2026. Le modifiche sotto
sono proposte per il rebuild locale o per una fase autorizzata successiva; non
sono state applicate alla VPS14 Azure.
...
4. Tenere MariaDB/Redis e worker on-demand, ma non spegnere PostgreSQL mentre
   la dashboard usa `azienda_lab`.
...
| PostgreSQL | cgroup 97 MiB; serve `azienda_lab` per dashboard, oltre a Forgejo/Nextcloud | TEMPORARY CORE; tuning solo dopo load test; futuro on-demand dopo migrazione dashboard AD/LDAP | 0 | dashboard down se fermato | sì | sì |
...
| Docker | daemon PSS 183 MiB, cgroup 203 MiB; `containerd` segue Docker; container live ~1.47 GiB | opzione A corrente: Docker CORE per AdGuard; container `restart: "no"`, controller-owned | ~1.35 GiB a boot dagli optional | app non disponibile finché non parte il profilo | sì | no |
...
PostgreSQL è CORE temporaneo: la dashboard corrente usa `azienda_lab`, oltre a
Forgejo e Nextcloud. `postgresql_core_reason: dashboard_current_dependency`.
Una futura dashboard AD/LDAP-primary potrebbe renderlo on-demand se nessuna
funzione CORE residua lo usa; non è implementato. MariaDB serve Pterodactyl;
Redis host serve Pterodactyl/queue e il Redis Docker serve Nextcloud. Il
controller lascia PostgreSQL acceso durante ogni switch e ALL-OFF-OPTIONAL non
lo ferma finché la dipendenza dashboard resta reale.

## Docker e boot

La discovery live ha otto container: AdGuard, Forgejo, Stirling, tre container
Nextcloud con `unless-stopped`, e due Scribble unmanaged con `no`. Il target IaC
usa `restart: "no"` per ogni servizio e delega l'avvio alle unità profile.
