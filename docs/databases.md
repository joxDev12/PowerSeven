...
I database sono su VPS14. Il cluster PostgreSQL reale è `postgresql@18-main.service` (18.6), mentre `postgresql.service` è solo l'aggregatore. PostgreSQL ascolta su `127.0.0.1:5432` e `10.10.10.14:5432`; database non-template osservati: `azienda_lab`, `forgejo`, `nextcloud`, `postgres`. MariaDB 10.11.14 ascolta su `127.0.0.1:3306`; `panel` è il database applicativo osservato di Pterodactyl. Redis di sistema ascolta su `127.0.0.1:6379`; esiste anche un Redis isolato nella rete Docker di Nextcloud (`soc-cloud-redis-1`).

| Motore | Consumatori | Autenticazione/storage | Health check read-only | Failure mode |
|---|---|---|---|---|
| PostgreSQL | Forgejo e Nextcloud dichiarati; `azienda_lab` osservato | gestito dal controller come `postgresql@18-main.service`; ruoli, password e TLS non inclusi | `pg_lsclusters`; `systemctl status postgresql@18-main.service` | applicazioni senza persistenza o login |
